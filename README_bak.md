...
├── .github/
│   ├── workflows/
│   │   ├── deploy-terraform.yaml         # Pipeline — provisions AWS infra + EKS via Terraform (Option A)
...
│   ├── secret-store.yaml                 # ESO SecretStore (Options A and D)
│   ├── external-secret.yaml              # ESO ExternalSecret (Options A and D)
...
| terraform | ≥ 1.6 | [docs](https://developer.hashicorp.com/terraform/install) — only needed for Option A |

> **Kubernetes cluster:** ARC requires a running Kubernetes cluster. If you do not have one, Step 4 includes instructions to create one manually via the AWS CLI, or you can use **Option A** under step 4 to provision everything via Terraform.
...
The deploy pipelines authenticate to AWS via OIDC — no static credentials. The OIDC provider must exist in your AWS account before anything else can run — both the `deploy-arc.yaml` pipeline and Terraform (Option A) depend on it.

> ⚠️ **Step 2 is required regardless of which option you choose in Step 4.** Options B, C, and D do not use OIDC directly, but the deploy pipelines (`deploy-arc.yaml` and `deploy-terraform.yaml`) authenticate to AWS via this OIDC role. Option D additionally requires the OIDC provider to exist because Terraform references it via a `data` source.
...
### 🟠 Step 3 — Create an S3 bucket for Terraform remote state (Option A only)

Skip this step if you are using Options B, C, or D.

Terraform requires an S3 bucket to store remote state before `terraform init` can run. Create it once per account:
...
Then update the `backend "s3"` block in `terraform/providers.tf`:
...
### 🟡 Step 4 — Provision the cluster and create the Kubernetes secret

> ✅ **This project uses Option A under this step.** The `terraform/` directory contains all the infrastructure code. Options B, C, and D are for secret management — choose whichever suits your setup.
...
**If you already have a Kubernetes cluster**, skip straight to the option that suits you (B, C, or D).
...
- Or use **Option A** which provisions the cluster AND the secret via Terraform in one shot

---

#### 🟡 Create an EKS cluster manually (skip if you already have one, or if using Option A)
...
#### 🟡 Option A — Terraform (provisions everything: EKS cluster + IAM + secrets + ESO)

Use this option if you want all AWS infrastructure managed as code. Requires Steps 2 and 3 (OIDC provider and S3 bucket) to be completed first. Skip the manual cluster creation above — Terraform handles it.
...
**Option 1 — Run Terraform locally (when testing):**
...
The `deploy-terraform.yaml` pipeline runs Terraform automatically when changes are made at `terraform/**` and `deploy-terraform.yaml`. Before pushing, set these GitHub Actions secrets:
...
**After `terraform apply` completes**, the outputs (cluster name, region) are printed automatically — use them for the GitHub Actions secrets in Step 5.

---

#### 🟡 Option B — kubectl (quick, local dev)
...
#### 🟡 Option C — Sealed Secrets (GitOps-safe, recommended for teams)
...
#### 🟡 Option D — External Secrets Operator + AWS Secrets Manager (production)
...
Run it after completing Steps 1–5 (GitHub App, OIDC, S3, secrets, etc.) with your kubeconfig pointing at the target cluster:
...
**If using manual setup (Options B/C/D):**
