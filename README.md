...
➡️ **Controller pod is CrashLoopBackOff**
```bash
...
➡️ **Runner pods stuck in Pending or CrashLoopBackOff with IP errors**
```bash
...
➡️ **Terraform: "Addon already exists" or "cannot re-use a name that is still in use"**
...
➡️ **ESO SecretStore: "resource is not valid for cluster"**
...
➡️ **Node group CREATE_FAILED: unhealthy nodes**
...
➡️ **EKS console shows "No Nodes" or kubectl returns "provide credentials"**
...
➡️ **NetworkPolicy blocking runner traffic**
```bash
